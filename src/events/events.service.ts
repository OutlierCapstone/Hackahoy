...
export class EventsService {
  ...
  async trackUserEvent(userId: string, problemId: number, type: UserEventType) {
    try {
      ...
      if (!problemExists) {
        console.warn(
          `[EventsService] 존재하지 않는 문제(ID: ${problemId})에 대한 이벤트 기록 건너뜀.`,
        );
        return null;
      }
      ...
      console.error(
        `[EventsService] userEvent 생성 중 예상치 못한 에러 - userId: ${userId}, problemId: ${problemId}`,
        err.message,
      );
      return null; // 서버가 중단되지 않도록 예외 처리
    }
  }
