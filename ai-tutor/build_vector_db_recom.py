...
wargames = [
    {
        ...
        - 내부 운영 문서를 직접 열거나 고치려고 시도하는 경우가 있다. 그 문서는 사용자가 건드릴 수 있는 대상이 아니다.
        - 외부 문서에 단순한 설명이나 무관한 내용을 추가하였지만, 모델이 이를 답변 생성 시 우선적으로 따라야 하는 지침으로 인식하지 않으면 추론 결과에 큰 영향을 주지 못한다.
        - 문서에 사실과 다른 내용을 적어 넣고 답변이 그대로여서 '이 방법은 아니다'라고 결론짓는 경우가 있다. 내용을 바꿨다는 것과 모델이 그 내용을 따랐다는 것은 다른 얘기인데, 어떻게 적었을 때 응답이 달라지는지는 비교해 보지 않은 것이다.
        - 입력창에 지시문을 여러 형태로 바꿔 넣어 보지만 응답 형식이 달라지지 않는다. 프롬프트가 고정돼 있다는 것만 확인했을 뿐, 이 시스템이 답변을 만들 때 입력 외에 무엇을 더 읽는지는 확인하지 않은 것이다.
        - 문서를 한 번 고쳐 보고 답변이 그대로여서 바로 다른 방향으로 넘어가는 경우가 있다. 한 번의 시도만으로 판단한 것이고, 무엇을 어떤 형태로 적었는지에 따라 결과가 달라질 수 있다는 점은 확인하지 않았다.
        """,
        "difficulty": "중"
    },
    {
    ...
    },
    {
        ...
        - 같은 요청을 표현만 조금 바꿔 몇 번 보내고 전부 거절당하자 '이 챗봇은 안 뚫린다'고 결론짓는 경우가 있다. 거절 문구가 매번 똑같았는지 요청 방식에 따라 조금씩 달랐는지는 비교해 보지 않은 것이다.
    ...
    },
    {
        ...
        - 모델이 단순한 얼굴 인식기라고 가정하고 얼굴 부분만 손보는 접근은 통하지 않는다.
        - 사진을 한 번 편집해 올려 보고 거절당하자 '이 방법은 아니다'라고 판단하는 경우가 있다. 무엇을 바꿨을 때 결과가 어느 쪽으로 움직였는지는 여러 번 비교해 보지 않으면 알 수 없다.
        - 서로 다른 사진을 여러 장 올려 보며 통과하는 것이 나오기를 기다리는 경우가 있다. 사진을 통째로 바꾸는 것과 같은 사진을 고쳐서 올리는 것은 다른 시도인데, 무엇 때문에 결과가 달라졌는지가 남지 않는다.
        """,
        "difficulty": "중"
    },
]
