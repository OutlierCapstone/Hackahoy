...
wargames = [
    ...
    },
    {
        ...
        """,
        "wrong": """
        - 로그인 폼에서 관리자 계정의 비밀번호를 추측하거나 무차별 대입을 반복하는 경우가 있다. 이 문제는 인증을 뚫는 문제가 아니라, 이미 발급된 토큰을 서버가 어떻게 취급하는지를 보는 문제다.
        - 유효한 서명을 다시 만들어야 한다고 전제하고 시크릿 키를 찾는 데 시간을 쓰는 경우가 있다. 요청이 거부되는 이유가 서명 때문인지 다른 값 때문인지를 응답 메시지로 구분하지 않은 상태다.
        - Payload를 base64 디코딩해서 role 값이 보이는 것을 확인하고 그 자체를 취약점이라고 결론 내리는 경우가 있다. 값이 보이는 것은 취약점이 아니고, 바꾼 값을 서버가 받아들여야 성립한다.
        - 토큰의 가운데 조각을 손으로 편집하다 base64 패딩이나 JSON 구조가 깨져서 요청이 거부되는 경우가 있다. 디코딩 자체가 실패한 응답과 권한이 없다는 응답은 서로 다른 메시지이므로 구분해서 읽어야 한다.
        - Authorization 헤더에 Bearer 접두어 없이 토큰만 넣어 보내 토큰이 없다는 응답을 받는 경우가 있다. 서버가 헤더의 어떤 형식을 기대하는지 확인하지 않은 상태다.
        - role 값을 Admin, ADMIN, administrator 처럼 다른 표기로 바꿔서 여전히 거부되는 경우가 있다. 서버는 문자열을 엄격하게 비교한다.
        - localStorage의 다른 키나 화면에 표시되는 역할 문구를 바꿔서 관리자 화면이 나오기를 기대하는 경우가 있다. 화면 표시가 바뀌어도 서버가 내려주는 응답은 바뀌지 않는다.
        - 변조한 토큰을 로그인이나 회원가입 요청에 다시 보내는 경우가 있다. 권한 검증이 일어나는 엔드포인트로 보내야 의미가 있다.
        """,
        "difficulty": "중"
    },
    {
