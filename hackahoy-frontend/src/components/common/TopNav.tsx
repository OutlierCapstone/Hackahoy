...
export default function TopNav({
  ...
  const renderNavButtons = (buttons: NavButton[]) => {
    return buttons
      .filter((btn) => {
        if (btn.type === "admin") {
          const adminCheck =
      ...
      .map((btn, idx) => renderBtn(btn, idx));
  };

  return (
    <header className={styles.bar}>
      <nav className={styles.inner}>
        <div className={styles.side}>{renderNavButtons(left)}</div>
        <div className={styles.sideRight}>{renderNavButtons(right)}</div>
      </nav>
    </header>
  );
}
